...
public class SimpleMonitorService implements MonitorService {
	...
	public SimpleMonitorService() {
		queue =
			new LinkedBlockingQueue<URL>(Integer.parseInt(ConfigUtils.getProperty("dubbo.monitor.queue", "100000")));
		...
		JFreeChart jfreechart =
			ChartFactory.createTimeSeriesChart(
				"max: " + numberFormat.format(summary[0])
					...
					+ (summary[3] >= 0 ? " sum: " + numberFormat.format(summary[3]) : ""), toDisplayService(service)
					+ "  " + method + "  " + toDisplayDate(date), key, xydataset, true, true, false);
	...
	private void write() throws Exception {
		...
		for (String key : types) {
			try {
				...
				String filename =
					statisticsDirectory + "/" + day + "/" + statistics.getServiceInterface() + "/"
						+ statistics.getParameter(METHOD) + "/" + consumer + "/" + provider + "/" + type + "." + key;
	...
	private void writeTo() throws Exception {
		...
		try {
			...
			dubboInvoke.setInvokeDate(sdf.format(now));
			...
			// 写入数据库
			String sql =
				"INSERT INTO dubbo_tb_invoke (PROVIDER, CONSUMER, SERVICE, METHOD, TYPE, INVOKE_DATE, INVOKE_TIME, SUCCESS, FAILURE, ELAPSED, CONCURRENT, MAX_ELAPSED, MAX_CONCURRENT, STATE, CREATE_DATE,  CREATE_USER, MODIFY_DATE, MODIFY_USER)"
		...
		for (File dateDir : dateDirs) {
			...
			for (File serviceDir : serviceDirs) {
				...
				for (File methodDir : methodDirs) {
					String methodUri =
						chartsDirectory + "/" + dateDir.getName() + "/" + serviceDir.getName() + "/"
							+ methodDir.getName();
					...
					for (File consumerDir : consumerDirs) {
						...
						for (File providerDir : providerDirs) {
							...
							appendData(new File[] { consumerSuccessFile, providerSuccessFile }, successData,
								successSummary);
							...
							appendData(new File[] { consumerElapsedFile, providerElapsedFile }, elapsedData,
								elapsedSummary);
							elapsedMax =
								Math.max(elapsedMax,
									CountUtils.max(new File(providerDir, CONSUMER + "." + MAX_ELAPSED)));
							elapsedMax =
								Math.max(elapsedMax,
									CountUtils.max(new File(providerDir, PROVIDER + "." + MAX_ELAPSED)));
					...
					if (elapsedChanged) {
						...
						elapsedSummary[3] = -1;
						createChart("ms/t", serviceDir.getName(), methodDir.getName(), dateDir.getName(), new String[] {
							CONSUMER, PROVIDER }, elapsedData, elapsedSummary, elapsedFile.getAbsolutePath());
					}
					if (successChanged) {
						...
						createChart("t/s", serviceDir.getName(), methodDir.getName(), dateDir.getName(), new String[] {
							CONSUMER, PROVIDER }, successData, successSummary, successFile.getAbsolutePath());
					}
				}
			}
		}
